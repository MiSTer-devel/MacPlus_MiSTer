...
// order: the OSD parser cannot leave a numbered gap, and renumbering would
// change what a saved config boots.
//
module mac_model
(
	...
	// is clear the ROM window MIRRORS at A17 = 1, and that mirroring is how a
	// machine SAYS it has no SCSI: the Plus ROM compares $420000 against
	// $440000 ($4003E4), equal means no SCSI, and it records that in $0B22
	// bit 7, which gates every later SCSI access. It is the only thing that
	// distinguishes a 512Ke from a Plus, since they run the same ROM. Clear
	// also stops $58xxxx decoding, so software that pokes the chip directly
	// finds nothing. The SE needs no special case: its ROM is 256K, so A17 is
	// a real address bit and the two probes differ in content, not decode.
	...
	// slot as a plain binary count in ioctl_index[7:6] (`i << 6` for i = 0..3),
	// so a slot number here IS the boot file number.
	...
	always @(*) begin
		case (model)
			...
			// The 512K and 128K run the 64K ROM in slot 2. RAM is soldered,
			// and its size is the one hardware difference between them.
			// scsiPresent = 0 is accuracy rather than necessity: the 64K ROM
			// has no SCSI Manager and never scans the bus. drive800k = 0
			// because both shipped a mechanically single-sided drive, so an
			// 800K image cannot be read at all.
			MODEL_512K: begin
